...
- **External Users** Hobbyists, through to learning and budding developers.
    ...
    3.  As a developer:
        - I'm always looking and open to trying tools that will make my work more efficient and save me time.
...
- **The Owner** That would be me in this case.
    ...
    1.  As the owner:
        ...
        - I want to create it to my design preferences, and to be able to easily change it when I want to.
...
Having played on a few of these sorts of sites myself, I know what I find interesting. Also, what makes it an experience to which I would return. Given the limit on time, and the abundance of ideas, my challenge is to confirm the scope to what is possible so the project can be submitted in time.
...
- **Functional Requirements**
    ...
    - **Range Selectors** The value will be displayed when the user changes it. Default and minimum and maximum values will be set for different attributes as a guide for the user.
    ...
    - **Navbar** Fixed navbar, with a toggle button so users will be able to open the off-canvas, and see a list of useful resources.
    - **Copy to clipboard** Users will be able to easily copy the CSS style and there will be visual feedback to confirm the copy is successful.
    - **Gradients** Users will be able to select colors to create gradients. A start color, and end color, and a direction select will be used to compile CSS rules for cross-browser compatibility.
    - **Contact form** Users will be able to access the contact form from the navbar or footer. By clicking on the email icon will open the modal with the contact form. It will include validation and they will be able to send a message. A success message will be displayed to let the user know the message has been sent.
    - **Accordion** There will be multiple levels of forms and fieldsets of inputs grouped so they can be compiled into CSS rules. The user will be able to work through the different style attributes and see the result live as they change the style values.
    - **Demo button** The user will be able to add their button text, colors, style attributes by changing the values of the form inputs. The change in values will be updated live to the demo button. The user will be able to change the background color of the button so they see what the style looks like against the intended background for the button.
...
- **Interaction Design**
    ...
    - The user will be able to play, create a button, and be able to easily copy it to be able to use the exact style where they want to use it.
...
- **Wireframing:**
    ...
    I combined the 3 separate wireframes into one image for the README as seeing them side by side gives a better perspective of the scale of difference between the different size screens The original size wireframes are available for viewing in the document folder. 
...
3.  [Google Fonts](https://fonts.google.com/)
    - A selection of Google Fonts was used in the “select” input for selecting CSS font style for the button styling.
...
5.  [Git Version Control](https://git-scm.com/)
    - Used to commit blocks of work to the project. Also created branches for working on specific changes or testing.
6.  [Clipboard.js](https://clipboardjs.com/)
    - Used to include copy (add to clipboard) functionality for the user to copy the CSS markup.
...
- **Visual Studio Code - Live Server*
    ...
    - I found it beneficial to be able to view the live changes on saving the work. It meant that I could monitor for mistakes and fix them knowing where they were arising, rather than looking for them.
...
3.  At this point I started to look at the form fieldsets of inputs and work out how to generate the CSS output and styling. The first fieldset was pretty straightforward, as the inputs individually generated single CSS rules. The font family (a selection of 10 stylish google fonts listed in a select), font color (color picker), and font size (range selector). My initial idea was to create spans and place them within `<pre></pre>` tags within a div beside the accordion with the forms. The CSS rule was created, placed in the corresponding span, and the CSS rule also added to the display button using Javascript. Then I came to the second fieldset, for font-shadow, and I needed to combine the values from 4 inputs, that are initially disabled, to output the CSS and style. I need to research and learned that I would not be able to implement the hover as it's not possible to apply pseudo-classes on an element directly with Javascript.
...
So my initial strategy was to research, and I used Codepen to create pens and implement working features there so I could learn how to do it, and what was needed before jumping into writing code. There are several pens where you can see what I was playing with, and understand then what I have put into the site. 
...
- [Bootstrap 4 color pickers](https://codepen.io/daidensacha/pen/jOyVrqW) This was my initial choice, but I moved away from it after experiencing some conflicts and bugs that I couldn't work out. With limited time, my choice to use jscolor color picker was a good one. 
- [Bootstrap Offcanvas](https://codepen.io/daidensacha/pen/QWdvyBd) is based on a demo version from Bootstrap and was used in my site.

I made a lot of my mistakes playing in Codepen, working things out, and learning a lot in the process. With this knowledge, I proceeded with the site.  

**Form HTML**
The HTML came together really quickly and seamlessly. The accordion with 5 levels and 33 form elements in it, took a little while, but the whole page was done in a day, with all form elements added, and viewing perfectly on all screen sizes. Chrome developer tools with the ability to switch between different types of devices are really helpful.
...
-  I had issues disabling fieldsets of inputs with the toggle buttons using JavaScript. It worked, however, the color pickers were still clickable and would open, even though the input was disabled. In the end, I discovered that jQuery did the trick, and although Javascript was the primary code used, I used jQuery in a few places where it was needed. 
    -  **Addendum**: I thought this was behind me until I ran my code through the validation. One warning was that I had used `input disabled="true"`, and that it is no longer used, so I changed it to the recommended `input disabled`. My problem came back with the color picker being disabled but popping up on click when disabled. It turned out that I had removed the disabled from one altogether, and was relying only on the group class name and jQuery to disable the group. When I added `input disabled` back in, it worked fine, so was a relatively easy bug to sort out.
-  **How to set CSS styles and display the CSS Markup** 
I started by applying the CSS Styling directly to the demo button. It was pretty straightforward with applying styles from individual inputs. But when I reached the second fieldset and was faced with compiling CSS markup from a combination of fields, it stumped me. through research, I also discovered that pseudo CSS styles cannot be applied directly to elements using JavaScript. 
My solution, in the end, was to scrap my initial JavaScript and to start again but with a different strategy. Create style tags, append them to the head, and then set the css rules directly to those tags. All inputs have an event listener with a .update() function so they update oninput, or onchange, depending on the input. This is all in the form.js, where I assign variables to the inputs, the toggle switches, color pickers, the demo button, the CSS output display block. I used literal notation to create strings with the die=fferent values from inputs in fieldsets to create rules Using if statements, each fieldset passed with the function and if the syle is to be included it is pushed to an array. Here the order needed to be sequential, so the output was in order. 
What brings it together in the end, is to assign the array to a variable using this statement. `let css = lines.join("\n");` The strings within the array are joined with the new line escape, so the rules display per normal CSS. I needed to add `\t` at the beginning of each string that was then assigned to a variable. 
This was the major part of the work, and it was a week of highs and lows, long hours, despair, however, in the end, I'm really happy with how it turned out. 
...
I researched various options and concluded that clipboard.js was the best option due to cross-browser compatibility, and functionality. Their website has clear documentation, and I also found a few other sources with examples showing how to implement the clipboard. 
    ...
    - [clipboard.js](https://clipboardjs.com) documentation shows how to handle the event and display the success message or error message. This was cool, but the message was gone in a flash and not user-friendly. I added a timeout so it disappears after 2 seconds. The coloring is accomplished by adding class names styled specifically for the message display. 
- **CSS Display background issue**
The CSS display block is for good reason next to the accordion with the styling inputs. The CSS markup can vary, from half the height of the accordion to almost full height. With no background, it leaves a space that just didn't look right. I added a background color to the CSS block, but it included the background of the container padding that is normally invisible. It had the effect of looking out of balance, as the accordion to the left had invisible padding. It took me a while to find out what was causing it, and my solution, in the end, was to move up a level and add the background color to the whole row, so it no longer looks out of balance, and the CSS display looks ok if the markup is not filling the display block.
- **Git Version Control**
`git status`, `git add .`, `git commit -m ""`, and `git push` was regularly used. I tried to use it as I worked with chunks, so the commits were commented on in stages. For this project, especially when it came to developing the `form.js` to handle the form input data and output the CSS styles, I decided to create a branch, so that if things went haywire then I had a place reserved to start from afresh. Create the branch `git checkout -b add-color pickers. It was my first time using this, and it went pretty seamlessly. When I finished and was happy, I sat on it for a few days in fear of losing the work, not knowing what would happen when I merged it to the master. In the end, I bit the bullet, and Git did not complete the merge but showed me the files with common code shown, and the differences in code from the two files, so I could choose what to merge and what not to. It was seamless and I had no real issues. It was a good process, and the experience showed me how to use the git branch. There is a lot to learn, but it is brilliant how it works.  
